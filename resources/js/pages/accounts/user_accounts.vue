  ...
  <v-card>
    ...
    <v-card-title class="align-end">
      ...
      <v-text-field
        ...
        hide-details
      ></v-text-field>
      <v-btn color="#ffcc6c" @click="deleteAll">Delete</v-btn>
    </v-card-title>
    <v-data-table
      :headers="headers"
      :items="items"
...
<script>
  export default {
    ...
    methods:{
      ...
      getUser(){
        this.axios.get('/admin/client').then((response) => {
      ...
      deleteAll(){
        if(this.datas.length == 0) return this.$awn.warning('No selected account to be deleted')
        let notifier = this.$awn;
          let onOk = () => {this.deleteConfirmAll()};
          let onCancel = () => {};
          notifier.confirm(
            'Are you you want to delete this selected Accounts?',
            onOk,
            onCancel,
            {
              labels: {
      ...
      deleteConfirm(item){
        let thiss = this;
        this.$awn.asyncBlock(
          thiss.axios.delete('/admin/client/'+item.id).then((response) => {
             thiss.getUser();
          }),
      ...
      deleteConfirmAll(){
        let thiss = this;
        this.$awn.asyncBlock(
          thiss.axios.post('/admin/client/deleteall',thiss.datas).then((response) => {
           thiss.getUser();
          }),
          'Selected Accounts delete successfully'
        ) 
      },
    }
  }
